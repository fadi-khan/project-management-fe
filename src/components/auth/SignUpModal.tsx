...
import { signupValidationSchema } from "@/lib/validations/schemas/signupValidations"
...
export const SignupModal = () => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        await handleSubmit(
            async (validatedData: { email: string; password: string ;  name:string}) => {
            ...
            <div
                ...
                <form onSubmit={handleLogin} className=" flex flex-col items-center mt-6 justify-center gap-y-6">
                     ...
                     <Field className={"flex-col flex w-full "}>
                        <Label className={"font-medium  text-blue-900"}>Name <span className="text-red-500">*</span></Label>
                        <div className="flex px-3 items-center border border-blue-900 bg-white rounded transition focus-within:ring-2 focus-within:ring-blue-900 focus-within:border-blue-900">
                            <Input
                                ...
                                placeholder="Enter you name" required type="text"
                                className={"w-full border-none  p-2  "}>

                            </Input>
                            <FaUser className="text-blue-900" size={20} />
                        </div>
                        <Label className={"text-red-500 text-sm font-medium mt-1"}>{hasFieldError('name') && getFieldError('name')}</Label>

                    </Field>

                    <Field className={"flex-col flex w-full "}>
                        <Label className={"font-medium  text-blue-900"}>Email <span className="text-red-500">*</span></Label>
                        <div className="flex px-3 items-center border border-blue-900 bg-white rounded transition focus-within:ring-2 focus-within:ring-blue-900 focus-within:border-blue-900">
                            <Input
                    ...
                    <Field className={"flex-col flex w-full "}>
                        <Label className={"font-medium text-blue-900"}>Password <span className="text-red-500">*</span></Label>
                        <div className="flex px-3 items-center border border-blue-900 bg-white rounded transition focus-within:ring-2 focus-within:ring-blue-900 focus-within:border-blue-900">
                        ...
                        <label className="w-full text-start text-sm text-gray-500  pt-2 cursor-pointer ps-0.5">Already have an account? 
                            <Link href="/login" className="text-blue-900 font-medium"> Login</Link>
                        </label>
                    ...
                    <Input disabled={isSubmitting === true}
                        ...
                        type="submit" name="Login" value={isSubmitting === true ? "Submitting..." : "Sign up"}></Input>
                </form>
            </div>
        </div>
